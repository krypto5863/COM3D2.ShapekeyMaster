...
namespace ShapekeyMaster
{
	internal class ShapekeyFetcherSetter
	{
		...
		private static List<TMorph> FixList = new List<TMorph>();
...
#if (DEBUG)
			Debug.Log($"Checking maid isn't null.");
#endif
...
#if (DEBUG)
			Debug.Log($"Fetching morphs");
#endif

			//var morphs = await Task.Factory.StartNew(() => GetAllMorphsFromMaid(maid));
			var morphs = GetAllMorphsFromMaid(maid);


#if (DEBUG)
			Debug.Log($"Converting morphs to changes...");
#endif
...
#if (DEBUG)
			Debug.Log($"MissionControlMaid finished processing in {stop.Elapsed} ms");
#endif
		}
		...
		public static List<ShapekeyChangeForm> GetTMorphChangeForm(TMorph m)
		{
			...
			var resultList = new List<ShapekeyChangeForm>();
		...
		public static void ProcessChanges(IEnumerable<ShapekeyChangeForm> changes) 
		{
			...
			{
#if (DEBUG)
				Debug.Log($"There were 0 changes to be made...");
#endif
			}

			foreach (ShapekeyChangeForm s in changes)
			{

#if (DEBUG)
				Debug.Log($"Running foreach shapekey change on {s.ShapekeyName}");
#endif

				//taskList.Add(Task.Factory.StartNew(() => RunShapekeyChange(s.Morph, s.Index, s.Deform)));
				RunShapekeyChange(s.Morph,s.Index,s.Deform, s.ShapekeyName);
				//Main.@this.StartCoroutine(RunShapekeyChange(s.Morph, s.Index, s.Deform));
			}
			...
			/*This function is unsafe and causes crashing.. The above function seems to work fine despite them being functionally the same... Why? Who knows.
			Parallel.ForEach(changes, (s) => 
			{
				RunShapekeyChange(s.Morph, s.Index, s.Deform);
			});*/
		}
		...
		public static void RunShapekeyChange(TMorph m, int shapekeyIndex, float deform, string keyname) 
		{
			//yield return new WaitForEndOfFrame();

			//Setting a value of less than 0 can cause the game to crash. As such, we ensure we never set a 0 val.

			if (m != null && m.GetBlendValues(shapekeyIndex) != deform)
			{
				if (deform < 0)
				{
					deform = 0;
				...
				/*if (!FixList.Contains(m))
				{
					FixList.Add(m);
				}*/

#if (DEBUG)
				Debug.Log($"Checking if was a face morph and needs updating");
#endif
				if (m.bodyskin != null && m.bodyskin.body != null && m.bodyskin.body.Face != null && m.bodyskin.body.Face.morph != null && m.bodyskin.body.Face.morph.Contains(keyname))
				{
					m.bodyskin.body.Face.morph.FixBlendValues_Face();
				}

#if (DEBUG)
				Debug.Log($"Done with single shapekey change.");
#endif
			}
		}
		/*
		...
		public static IEnumerator TaskQueuer() 
		{
			while (true)
			{
				...
				try
				{
					if (FixList.Count > 0)
					{
						foreach (TMorph m in FixList)
						{
#if (DEBUG)
							Debug.Log($"Fixing blend vals!");
#endif
